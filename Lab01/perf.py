...
if __name__ == "__main__":
    for length in [5, 25, 125, 625, 3125, 15625, 78125]:
        ...
        vec1 = np.random.rand(length)
        print("Mean vector (Pure C++), size={0}: {1} seconds".format(length, test_timings(CosinDistance.get_cosin_distance, vec1, vec2)))
        print("Mean vector (Python scipy), size={0}: {1} seconds\n".format(length, test_timings(distance.cosine, vec1, vec2)))